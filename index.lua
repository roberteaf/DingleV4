...
<head>
    ...
    <script type="module">
        ...
        import { getFirestore, collection, doc, getDoc, setDoc, updateDoc, addDoc, onSnapshot, query, orderBy, limit, deleteDoc } from 'https://www.gstatic.com/firebasejs/12.4.0/firebase-firestore.js';
        window.firebaseApp = { initializeApp, getFirestore, collection, doc, getDoc, setDoc, updateDoc, addDoc, onSnapshot, query, orderBy, limit, deleteDoc };
    </script>
    <style>
...
<body>
    ...
    <script type="module">
        console.log('Minecraft Gamble Hub v2 - NEW Firebase + burst123 FIXED 🎮💎');

        // === NEW FIREBASE CONFIG (your fornite-d9d6b project) ===
        const firebaseConfig = {
          apiKey: "AIzaSyDRuEmGmNtB7MQle8ENzSignWH1V40qfpA",
          authDomain: "fornite-d9d6b.firebaseapp.com",
          projectId: "fornite-d9d6b",
          storageBucket: "fornite-d9d6b.firebasestorage.app",
          messagingSenderId: "78189590137",
          appId: "1:78189590137:web:bc82f5320a7570e5e6c6a8",
          measurementId: "G-CGPPYQKJ04"
        };

        const { initializeApp, getFirestore, collection, doc, getDoc, setDoc, updateDoc, addDoc, onSnapshot, query, orderBy, limit, deleteDoc } = window.firebaseApp;
        const app = initializeApp(firebaseConfig);
        const db = getFirestore(app);

        // === ADMIN FIXED (burst / burst123) ===
        const ADMIN_USERNAME = 'burst';
        const ADMIN_PASSWORD = 'burst123';

        async function initAdminAccount() {
            const adminRef = doc(db, 'users', 'burst');
            ...
            if (!adminSnap.exists()) {
                ...
                console.log('✅ Admin account created (burst / burst123)');
            } else {
                await updateDoc(adminRef, { password: 'burst123' });
                console.log('✅ Admin password updated to burst123');
            }
        }
        initAdminAccount();
        ...
        function loadMessages() {
            ...
            messagesUnsub = onSnapshot(q, (snapshot) => {
                ...
                snapshot.forEach((docSnap) => {
                    ...
                    div.innerHTML = `<div class="message-content"><small>${new Date(data.time.toDate()).toLocaleTimeString()} - ${data.username}</small><br>${data.text}</div>`;
                    if (isAdmin) {
        ...
        function showAdminLogs() {
            ...
            if (logsDiv.style.display === 'none') {
                ...
                logsUnsub = onSnapshot(q, (snapshot) => {
                    let html = '<table><tr><th>Time</th><th>Type</th><th>User</th><th>Amount</th><th>MC Username</th><th>By</th><th>Action</th></tr>';
                    snapshot.forEach((docSnap) => {
                        const data = docSnap.data();
                        html += `<tr><td>${new Date(data.time.toDate()).toLocaleString()}</td><td>${data.type}</td><td>${data.user || '-'}</td><td>$${formatBalance(data.amount)}</td><td>${data.mcUsername || '-'}</td><td>${data.by}</td><td><button class="delete-btn" onclick="deleteLog('${docSnap.id}')">Delete</button></td></tr>`;
                    });
                    html += '</table>';
        ...
        async function deposit() {
            ...
            const amt = parseBet(amtStr);
            if (isNaN(amt) || amt <= 0) {
                showNotification('Invalid amount! ⚠️', 'error');
                return;
            }
            const mcUser = prompt('Enter your MC username |NOTE THIS IS UR MC USER| :');
            if (!mcUser) {
                showNotification('Invalid MC username! ⚠️', 'error');
                return;
            }
            await addDoc(collection(db, 'adminLogs'), {
            ...
            showNotification(`Deposit request for $${formatBalance(amt)} with MC username ${mcUser} logged. Contact admin to complete. 💳`, 'info');
        }
        async function withdraw() {
            ...
            showNotification(`Withdrew $${formatBalance(amt)} with MC username ${mcUser}. Logged for admin review. 🏦`, 'success');
        }
        ...
        async function login() {
            try {
                ...
                let passwordMatch = data.password === password;
                if (!passwordMatch) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            try {
                ...
                storedEdition = localStorage.getItem('currentEdition');
            } catch (e) {
                console.warn('LocalStorage access denied:', e);
            }
